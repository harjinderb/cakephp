    ...
    <section class="content">
		<div class="box">
            <div class="box-header invoice-header">
				...
				<?php 
					  echo $this->Form->create('',['class'=>'','type'=>'GET','url' => ['controller'=>'Users', 'action' => 'sendInvoice', 'prefix' => 'parent',$users->uuid]]);
					  echo $this->Form->hidden('invoice_group',   ['value'=> $invoives[0]['invoice_group']]);
						//pr($invoives[0]['invoice_group']);die; 
					  echo $this->Form->button('<span>download Invoice</span>',['class' => 'btn btn-white-round']);
						// echo $this->Html->link(
						// 'download Invoice',
					...
					<div class="row">
						...
						<div class="col-md-6">
							<div class="invoice-info__border invoice-info__invoice">
								...
								<p><span class="w-180-in-blk"><b><?=__('Invoice Date :')?> </b></span><?=date('d-m-Y', strtotime($createddate));?></p>
										...
										if (in_array($finalData, $childAttandance)) {
											...
											?>
												<tr>
													...
													<td><?= date('H:i:s', strtotime($childAttandanceDatesheet[$finalData][0]['date_time_end'])); ?></td>
													<td><?= $Overtime ?></td>
													<td><?= "Present"?></td>
									...
									<div class="table-responsive">
										<table class="table table-bordered table-striped table-hover v-center cell-pd-15">
											...
											<tr>
												...
												?>
												<td><b><?= date('d-m-Y', strtotime($previousinvoice)).' '.'-'.' '.date('d-m-Y', strtotime( $users['contracts'][0]['start_date']));?></td>
												<?php } ?>
											</tr>
											<tr>
