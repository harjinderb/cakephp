<section class="content">
	<div class="container-fluid">
		<div class="block-header">
			<h2 class="text-uppercase"><i class="fa fa-user-circle-o"> </i><?=__('User Pofile')?></h2>
			<?= $this->Flash->render() ?>
		</div>
		<div class="row clearfix">
			...
			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				<div class="card data-table-outer">
					<div class="header">
						<h2 class="text-uppercase"><?=$this->Decryption->mc_decrypt($user->firstname,$user->encryptionkey).' '.$this->Decryption->mc_decrypt($user->lastname,$user->encryptionkey);?></h2>
					</div>
					<div class="body">
							...
							<div class="row">
										...
										<div class="input-file-outer">
											<div class="input-file-in">
												...
												if(!empty($this->request->getSession()->read('Auth.User.image'))){
														...
														echo $this->Html->link($this->Html->image(USER_PICTURE_FOLDER_URL_PATH.$user['uuid'].'/'.$user['image'], ['alt' => 'John','class' => 'img-responsive max-width-user-pic']),'',['escapeTitle' => false,]);
													}else{												
											...
											<button class="btn bg-blue input-file-btn"> <?=__('Change image')?>...</button>
										</div>
									</div>
								</div>
								<div class="col-sm-8 col-md-8">
									...
									<fieldset>
										...
										<div class="form-group">
											<label for="password"><?=__('Password')?> <i style="font-size:12px">(<?=__('Password must contain atleast one capital letter and alphanumeric characters')?>.)</i></label>
									...
									<div class="form-group text-right">
										<button type="submit" class="btn btn-lg bg-teal"><?=__('Save')?></button>
									</div>
