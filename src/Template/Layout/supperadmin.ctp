...
<html>
    <head>
        ...
        <title><?= __('Super Admin | Kindplanner')?> </title>
        <?= $this->Html->meta('icon') ?>
        <?= $this->fetch('meta') ?>
        <meta charset="UTF-8">
...
?> 
    ...
    <section>
        ...
        <aside id="leftsidebar" class="sidebar">
            ...
            <div class="menu">
                <ul class="list">
                    <li class="header"><?=__('MAIN NAVIGATION')?></li>users

                    <li class="<?php if($lastWord == 'dashboard'){echo "active" ;} ?>">
                        <?php echo $this->Html->link('<i class="fa fa-home nav-ico"> </i><span>'.__('Dashboard').'</span>',['controller' => 'users','action' => 'dashboard', 'prefix' => 'admin'],['id'=>'admindashboard','escape' => false]); ?>
                    </li>
                    <li class="<?php if (strpos($url, "users")!== false){echo "active" ;} if (strpos($url, "dashboard")!== false){echo "inactive" ;}?>">
                        <?php echo $this->Html->link(' <i class="fa fa-cog nav-ico"> </i><span>'.__('Manage BSO').'</span>',['controller' => 'users', 'action' => 'index'],
                        ['escape' => false,'class'=>'']); ?>
                    </li>
                     <li class="<?php if (strpos($url, "users")!== false){echo "active" ;} if (strpos($url, "settings")!== false){echo "inactive" ;}?>">
                        <?php
                        //  echo $this->Html->link(' <i class="fa fa-cog nav-ico"> </i><span>Global Settings</span>',['controller' => 'users', 'action' => 'globalsettings'],
                        // ['escape' => false,'class'=>'']); 
                        ?>
                    </li>
